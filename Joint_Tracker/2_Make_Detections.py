...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose : # <-- if you wanna more highquality of tracking or detection change the numbers 
    ...
    while cap.isOpened():
        ...
        # draw_landmarks <- using draw utility from mp.solutions.drawing_utils

        # image                     <- drawing object
        # result.pose_landmarks     <- We stored out pose in Make Detection Sections. and then using Pose_landmarks tools // you can print it 
        # mp_pose.POSE_CONNECTIONS  <- we will connect all dots partitially // print it if you wanna see which part is connected
